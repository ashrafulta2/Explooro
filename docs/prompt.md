...
| 4 | Permission catalog + risk tiers | `PRD` §2.2, `idea` §3B | 0.4, 2.4 | 🟡 Partial — 0.4 done (`docs/rbac-spec.md` + `permission-catalog.json`: 182 permissions / 19 domains, 0 validation errors, all refs cross-checked against IA); 2.4 not started |
...
| 7 | Module registry (71 modules) | `idea` §4 | 0.7, 3.1, 3.2 | 🟡 Partial — 0.7 done (`docs/module-registry.md` + `server/src/config/modules.seed.json`: 71 modules / 9 groups, 0 validation errors, acyclic dependency graph, 65/65 source rows covered, all 36 IA module keys resolve); 3.1 and 3.2 not started |
...
| 12 | Auth: JWT, refresh rotation, OTP, 2FA | `DFD` 1.0 | 2.1–2.3 | 🟡 Partial — 2.1 done (`server/src/app.js`, `config/{env,db,cache}.js`, `config/cache-drivers/{redis,memory}.js`, `db/migrate.js`, `plugins/{errorHandler,requestContext}.js`); verified: `npm run dev` boots with `CACHE_DRIVER=memory` and no Redis running (console logs the active driver), a bad/missing env var fails fast with a readable per-variable list rather than a stack trace, `/api/v1/health` returns `ok` plus the active cache driver, a 404 and a thrown `AppError` both return the exact `docs/api-contract.md` §2.3 envelope with `trace_id`, `@fastify/rate-limit` is backed by the cache adapter (not its built-in store) via a custom Store class so limiting still works with no Redis, and `@fastify/helmet`/`cors`/`cookie` are registered. 2.2 done (`db/migrations/{001_identity,002_rbac,003_audit}.sql`, `db/seeds/{001_roles_permissions,002_dev_users}.sql`, `db/seed.js`, `lib/password.js`; generators `scripts/generate-{role-permission,dev-user}-seed.mjs`). Since no Neon/local Postgres is reachable from this environment, both migrations and the migration runner itself were verified for real — not just read — against `@electric-sql/pglite` (an embedded WASM Postgres) speaking the actual Postgres wire protocol via `@electric-sql/pglite-socket`, exercised as a transient dev-only tool, never added to the project: the real `pg` driver and the unmodified `migrate.js`/`seed.js` scripts ran against it. Confirmed: all 3 migrations apply cleanly on an empty database; `SELECT count(*) FROM permissions` = 182, exactly matching `docs/permission-catalog.json`; `role_permissions` = 358 rows; 7 dev users seeded (one per role) with real argon2id password hashes; the audit hash chain is correct (`prev_hash` of row 2 equals `row_hash` of row 1); `UPDATE`/`DELETE` on `audit_logs` both raise; re-running `migrate` and `seed` is idempotent; and `migrate.js`'s new checksum guard (retrofitted onto the 2.1 runner per `docs/erd.md` §13/§15, since it predates any real migration file) correctly refuses to proceed when an already-applied migration file is edited on disk, and accepts it again once restored. `avatar_media_id`'s FK to `media_assets` is deliberately deferred to Prompt 4.2, when that table first exists (documented in the migration itself). 2.3 done — `routes/auth.routes.js`, `controllers/auth.controller.js`, `services/{auth,otp,totp}.service.js`, `repositories/user.repository.js`, `integrations/sms/{index,mock,greenweb}.js`, `middlewares/authenticate.js`, plus supporting `lib/{jwt,encryption,ref,audit,rateBucket}.js` (small single-purpose helpers, not in the prompt's own FILES list but needed to satisfy its REQUIREMENTS — each documents why in its header). `jose` added for JWT (zero transitive deps, chosen over `jsonwebtoken`'s nine); TOTP (RFC 6238) and AES-256-GCM envelope encryption are hand-written, not new dependencies — see docs/dependency-ledger.md §4. `JWT_SECRET`/`COOKIE_SECRET`/`PII_ENCRYPTION_KEY` promoted from optional to required in `env.js`, with a shape check that `PII_ENCRYPTION_KEY` decodes to exactly 32 bytes. Endpoints: register, login, send-otp, verify-otp, refresh, logout, me, 2fa/setup, 2fa/verify — all 9 from the REQUIREMENTS list. Verified for real against the same `@electric-sql/pglite` + `pglite-socket` harness as 2.2 (transient, never added to the project), driving the actual Fastify app via `app.inject()` — not mocked: register → duplicate register correctly 409s → send-otp returns the code in `meta.otp_debug` only because `NODE_ENV=development` → wrong OTP code rejected, correct code verifies → password login issues a real access token plus `rt` (HttpOnly, `Path=/api/v1/auth`) and `csrf` (JS-readable, `Path=/`) cookies → `/auth/me` resolves roles/permissions/restrictions and 401s cleanly with no bearer → `/auth/refresh` 401s without a matching `X-CSRF-Token`, rotates the token when the header is present, and — the one that actually matters — **reusing the just-rotated refresh token revokes the whole session, and even the brand-new token issued right before the reuse is dead afterward**. Separately verified the 2FA gate end to end against the seeded `moderator` (a real account with MEDIUM/HIGH permissions from the catalog, not a synthetic fixture): password login returns `401 TWO_FACTOR_REQUIRED` with `details.challenge_token` and `enrolled:false` rather than any token; `/2fa/setup` completes using only that `challenge_token` — no Bearer token — which is what makes a staff account's very first enrollment possible at all, since login never issues a token to an unenrolled MEDIUM+ account in the first place; the generated TOTP secret was independently verified against the official RFC 6238 test vector before ever being used here; a wrong 6-digit code is rejected, the correct one completes the login and issues real tokens; a second login now reports `enrolled:true` and still requires a fresh TOTP code every time, not just once per account. Also verified: the compound rate-limit buckets (3/hour/phone, separate from 10/hour/IP) on send-otp trip independently — a 4th request for the same phone 429s with a `Retry-After` header while a different phone from the same IP still succeeds. Not built: password reset / COD_CONFIRM / PAYOUT_CONFIRM OTP purposes only consume generically for now, real handling belongs to checkout (Phase 6) and the vault (Phase 6); 2FA recovery-code redemption (codes are generated and hashed at setup, but no endpoint spends one yet) |
| 13 | RBAC resolution engine | `PRD` §2.2 | 2.4 | ✅ Done — `server/src/repositories/permission.repository.js`, `server/src/services/rbac.service.js`, `server/src/middlewares/{requirePermission,requireRestriction}.js`, `server/src/routes/me.routes.js`. Built by antigravity; reviewed and load-bearing bugs fixed in a follow-up pass (see row 14) — the resolution engine itself (6-step algorithm, cache versioning, DENY/GRANT/JIT layering, CRITICAL lockout) was solid and needed no changes, verified both by the existing `server/test/rbac.test.js` (mocked DB, 12/12 passing) and independently against a real Postgres connection (`@electric-sql/pglite`, transient dev-only tool): a JIT approval was visible on the *very next* request, not after the 5-minute TTL |
| 14 | **Delegation: grants, JIT, maker-checker** | *(new — not in any source)* | 0.4, 2.5, 3.3 | 🟡 Partial — 2.5 built by antigravity (`services/{delegation,makerChecker}.service.js`, `controllers/delegation.controller.js`, `routes/delegation.routes.js`, `jobs/grantExpiryCron.js`), then reviewed as a senior-dev pass and found to be **non-functional for its stated purpose** in its original form: three real bugs, all confirmed against a live Postgres connection (not the suite's mocked DB, which is why it hadn't caught them) and then fixed. (1) `PATCH /admin/pending-actions/:id` and `PATCH /access-requests/:id` were gated by `requirePermission('admin.approval.decide')`, itself HIGH-tier — a non-super-admin Admin's attempt to *decide* anything was recursively deferred into a *new* pending action instead of resolving the original, so only super_admin could ever approve anything. Fixed with a new `requirePermission(key, { skipMakerChecker: true })` option, used on both DECIDE routes, that checks held/not-held directly regardless of risk tier — confirmed live: Admin (not super_admin) now approves both a Mode C action and a Mode B request directly (200), not re-deferred (202). (2) Every APPROVE call 500'd: `getPendingAdminActionById(db, id, forUpdate: true)` ran `SELECT ... FOR UPDATE` across a query with `LEFT JOIN users a ON a.id = paa.approver_id` — Postgres categorically refuses to lock the nullable side of an outer join (`FOR UPDATE cannot be applied to the nullable side of an outer join`), reproduced standalone with zero application code involved. Fixed by scoping the lock to `FOR UPDATE OF paa`. (3) The one real HIGH-tier feature that exists yet (2.6's restrictions) had its executor registered under keys — `users.restriction.apply`/`users.restriction.lift` — that don't exist anywhere in `permission-catalog.json`; the actual gating key is `users.restriction.manage`, so `getRegisteredExecutor` always returned null and every approval silently marked the action `APPLIED` with zero effect: confirmed live — a restriction "approved" this way never appeared in `user_restrictions`. Fixed by registering one executor under the real key, disambiguating apply/update/lift via a `targetType` now threaded through the executor context (which previously carried neither `cache` nor `targetType`/`targetRef` at all — also fixed, since without a real `cache` the post-execution permission-cache invalidation was silently a no-op too). Also fixed in the same pass: the read-only `POST /admin/restrictions/preview` was gated by the same HIGH-tier key, so previewing (which mutates nothing) was needlessly routed through maker-checker for anyone but super_admin — moved to the LOW-tier `users.restriction.view`; and standing-grant expiry in `grantExpiryCron.js` bumped cache versions but wrote no audit row, unlike JIT/pending-action expiry, contradicting rbac-spec.md §3.1's "grant, revoke, and expiry each write an audit row" — now fixed. A submission-time "reject if no executor is registered" guard (rbac-spec.md §3.3 rule 3) was tried and reverted: it broke `server/test/delegation.test.js` and `rbac.test.js`'s own acceptance tests, which deliberately submit against permissions like `orders.refund.execute` that have no business-logic executor yet (built in a much later phase) specifically to test the routing mechanism in isolation — the missing-executor case is guarded at approval time only, where it actually matters. All 36 existing `node --test` cases plus a full live-Postgres regression (register→login→2FA→refresh-rotation-reuse, JIT approve-and-see-it-immediately, self-approval blocked at the DB level, Admin-approves-Moderator end to end with a real `user_restrictions` row created) pass after the fixes. Not changed, flagged for the team to decide: `POST/DELETE /admin/grants` (Mode A) are gated by CRITICAL-tier `users.permission.grant`/`revoke` (`default_roles: ['super_admin']` only, from the 0.4 catalog) — meaning standing grants are super_admin-only in practice today, whereas rbac-spec.md §8's own worked example has **Admin** Karim (not super_admin) creating one. Mode B's "requester cancels/releases early" (rbac-spec.md §3.2) still has no endpoint |
| 15 | **Granular per-user activity control** | *(new — not in any source)* | 0.4, 2.6, 3.3 | 🟡 Partial — 2.6 done, built by antigravity: `services/{restriction,segment}.service.js`, `controllers/restriction.controller.js`, `routes/restriction.routes.js`, `middlewares/requireRestriction.js` (2.4). All 4 enforcement modes (BLOCK/THROTTLE/FORCE_REVIEW_QUEUE/SHADOW_BAN), dynamic segment matching, and numeric-limit checks verified passing in `server/test/restriction.test.js` (7/7). This is also the one feature that exercises Mode C maker-checker for real — see row 14's fix log: its executor was wired to the wrong action_key and its dry-run preview endpoint was wrongly HIGH-tier, both fixed there. 3.3 (admin restriction-management UI) not started |
| 16 | Audit log + tamper evidence + explorer | `idea` §4 note | 2.7, 3.4 | 🟡 Partial — the `audit_logs` table itself (2.2's `003_audit.sql`) is done, ahead of its own prompt: full docs/erd.md §2 shape including `undo_payload`/`risk_tier`/`is_breakglass`, monthly `RANGE` partitioning (current month + 3 ahead, computed at migration time), a BEFORE INSERT hash-chain trigger (`row_hash = sha256(prev_hash \|\| row payload)`), and BEFORE UPDATE/DELETE triggers that raise on any mutation attempt — all verified against a real Postgres wire-protocol connection (see row 12). Writing an audit row from application code (the before/after-JSON helper every mutating service calls) and the admin-facing explorer UI (2.7, 3.4) are not started |
| 17 | Module control backend + targeting | `DFD` 8.0 | 3.1, 3.2 | ⬜ |
| 18 | Theme & Color Studio + 5 presets | `technologyused.md` §L1 | 3.5 | 🟡 Partial — **Master Colour engine added 2026-08-25 at the owner's request; Prompt 3.5's own ACCEPTANCE lines are still unaudited and this row does not claim them.** The reported defect was that a "1-click marketplace preset" changed buttons but left borders, hover fills, the scrollbar and the navigation chrome pink. Root cause measured, not guessed: components reference the RAW ramp steps (`--brand-50`…`--brand-1000`, `--neutral-0`…`--neutral-1000`) in **199 places across 15 files**, while `applyTheme()` overrode **33 semantic tokens and zero ramp steps** — so the pink ramp survived every preset switch. Re-theming had to regenerate the ramps, not repaint roles. New `client/src/services/colorRamp.js` derives the whole palette (brand + neutral + 4 status + accent, 45 steps) from one seed in OKLCH, reusing the exact lightness/chroma ladder transcribed from `themes.css` so `DEFAULT_MASTER` reproduces the shipped pink within 2/255 per channel (asserted by test). Seed hue drives the ramp; seed chroma scales the envelope; the anchor step is pinned to the seed itself so the admin's pick IS the button fill. Contrast decisions are computed, never assumed: button ink picks white or near-black by measurement, `--text-brand`/`--border-interactive` select the shallowest step clearing 4.5:1/3:1 on the generated canvas, and status ramps are lightness-repaired after chroma scaling. New `client/src/services/masterTheme.js` mounts the result as a `<style>` element mirroring themes.css's four blocks (base / OS-dark / explicit-dark / explicit-light) with doubled `:root:root` selectors. **This also fixed a second defect the original studio shipped with:** it pinned all 33 tokens as inline properties on `<html>`, which outrank `[data-theme='dark']` — measured live, a published legacy preset left dark mode completely dead (`--surface-0` identical in both themes) while the master path flips correctly. Per-section swatches now layer on top only where an admin actually moved one away from the generated value. **Third defect fixed:** `initTheme()` gated on `res.is_custom`, a field `theme.controller.js` never sends — so a published palette was fetched and silently discarded on every boot, i.e. publishing did nothing to the live site. **Fourth:** `client/index.html` carried an unconditional `body { background: #fbfdfe }` anti-FOUC rule that no stylesheet overrode, so the page canvas was a hardcoded near-white in every theme; `reset.css` now sets it from `--surface-0` via `html body`. The 6 legacy marketplace presets are retained but now route through the engine (their navbar/footer stay hand-authored as deliberate identity), sharing one `themeFromLegacyTokens()` with the boot path so studio preview and live site cannot disagree. 8 seed-only master presets in `client/src/config/master-themes.js`; Theme Studio gains a Master Colour panel (seed picker, harmony/tint/vividness controls, generated ramp readout); preview pane extended to the surfaces the old system could not reach (input boundary, hover fills, live scrollbar). Registered at `/dev/gallery` as `MasterPalette`. Verified: `client/test/colorRamp.test.js` — 9 invariant tests over 44 seed×variant palettes (ramp monotonicity, every studio-validated AA pairing, ≥3:1 input boundaries, dark theme distinct and ≥4.5:1, hover perceptibly different, malformed-seed fallback), all passing under `node --test`; measured live in-browser that 14/14 probed tokens plus `body` background change on seed switch and that dark mode survives. Budget gate after the change: initial JS 47.38KB gzip (budget 150), initial CSS 35.43KB gzip (budget 40) — CSS headroom is now thin. **Server-side validation of the master block closed 2026-08-26.** The gap mattered because the flattened sections describe light mode only, while what actually RENDERS is `regenerated palette ⊕ the section swatches an admin deliberately moved` — validating the submitted sections alone certified a theme the browser never shows. `server/src/services/masterPalette.js` is now the one server module permitted to import `client/src/services/colorRamp.js` (pure, DOM-free ESM; the same cross-workspace import `server/test/themeStudio.test.js` already used), so colour arithmetic exists in exactly one file — a second server-side copy of the OKLCH ladder would let the API certify a palette the browser renders differently. `theme.service.js` gains `validateMasterBlock()` (closed vocabularies, `MASTER_RANGES` bounds shared with the Studio's own sliders so the API can never refuse a value the UI offered, unknown-key rejection, and hard failure rather than `normaliseMasterConfig()`'s render-path fallbacks — a silently corrected seed means an admin publishes a colour they never picked), `deriveMasterTokens()` (regenerate → assert the dark-theme and input-boundary contracts the sections cannot express → flatten), and a `validatePalette()` that runs the WCAG gate on the merged effective palette. Section values must now be 6-digit hex, because `hexToRgb()` falls back to black for anything unparseable and junk would otherwise sail through the contrast gate as `#000000`. `saveDraft()` persists the NORMALISED master so what is stored is what was certified; `POST /admin/theme/validate-contrast` now returns the effective palette via `inspectPalette()`; `MASTER_CONFIG_INVALID` / `THEME_TOKEN_INVALID` / `THEME_MASTER_CONTRAST_FAILED` map to 422. `paletteToSectionTokens()` and `normaliseMasterConfig()` moved from `masterTheme.js` (which touches `document`) into `colorRamp.js` and are re-exported for existing importers. Verified: new `server/test/themeMaster.test.js`, 12 tests — all 8 master presets re-derive to themselves, malformed seeds/enums/bounds/unknown keys rejected with the offending field named, a failing pairing that exists ONLY in the derived⊕override merge is caught, non-hex sections rejected, dark-mode invariants asserted across 8 corner seeds × 3 neutral modes, legacy no-master palettes unchanged, plus route-level 422/201/200 behaviour through the real Fastify stack. Full suite after the change: client 9/9, server 377/378 (the one failure, `mediaPipeline.test.js` Acceptance 1, is unrelated and pre-existing — an uncommitted `/storage` → `/api/v1/storage` change in `server/src/integrations/storage/local.js`). Budget gate: JS 47.43KB gzip, CSS 35.43KB gzip, both PASS. **Flash-sale strip made themable 2026-08-26 at the owner's request:** its header, countdown chip and the FLASH tag on product cards painted from raw `--danger-300` / `--danger-800` with the ink hardcoded to `--neutral-900`, so no preset, no section control and no contrast check ever reached them. They now read five semantic tokens (`--flash-bg`, `--flash-text`, `--flash-chip-bg`, `--flash-tag-bg`, `--flash-tag-text`) authored across all four theme blocks in `themes.css`, generated per seed by `flashRole()` in `colorRamp.js`, flattened as a 7th `flash_sale` token section, and exposed as a Theme Studio accordion with a live strip in the preview pane. The ink is measured rather than assumed — `statusPull` leans the danger ramp's hue toward the seed, which moves its luminance — and the fill itself is repaired only if neither ink candidate clears 4.6:1, in which case a literal hex replaces the ramp reference. Three pairings joined the WCAG gate on both sides (`validatePaletteContrast()` and `theme.service.js`'s `runContrastChecks()`), with the countdown chip checked against the header's ink because the digits inherit it. Verified: the client AA test now covers the strip across all 44 seed×variant palettes plus both dark-theme pairings, and a 13th server test asserts every master preset generates a valid `flash_sale` section and that an override breaking the chip is rejected as `THEME_CONTRAST_FAILED`. Budget after the change: JS 47.87KB gzip, CSS 35.61KB gzip — both PASS, CSS headroom now 4.4KB. **Default colour switched to Midnight Slate 2026-08-26 at the owner's request** (pink kept as the second preset, every other preset untouched): `DEFAULT_MASTER_PRESET` now names `midnight_slate`, and `initTheme()` MOUNTS that default synchronously before asking the API for a published palette — previously it fell through to `clearThemeOverrides()`, which left the product wearing the authored themes.css baseline rather than any chosen default. `initTheme()` also moved to the front of the boot sequence in `main.js`, ahead of i18n, session and feature flags, so the shell stops wearing the CSS baseline for three round trips. The `/theme/active` mock returned a hand-pasted pink legacy palette flagged `is_published`, which meant mock mode overrode the shipped default on every boot and a developer could never see it; it now reports that nothing is published, which is what a fresh install looks like. Measured live on a clean dev server: `--brand` resolves to the seed itself (#334155), canvas #f7f8fa, borders neutral (surfaceWash/borderTint are both off for this preset), body text 15.4:1, button ink 10.1:1, input boundary 5.2:1, flash strip 8.4:1. **themes.css regenerated from the slate seed 2026-08-26 at the owner's request, closing the cold-load flash.** The CSS baseline still authored the PINK ladder by hand, so the frames before `initTheme()` mounted the real default painted a palette the product had stopped shipping. `styles/themes.css` is now GENERATED — `node scripts/palette.mjs --write` emits it from `DEFAULT_MASTER`, which moved to the slate config and is now asserted equal to `MASTER_PRESETS.midnight_slate.master`. `scripts/palette.mjs` was rewritten to consume the engine instead of carrying a second hand-typed OKLCH ladder (the duplication that made the drift possible), keeping only its independent WCAG/APCA maths so an engine bug cannot hide from it; its pairings are now named semantically and RESOLVED through the role maps, because which STEP backs a role moves with the seed — slate anchors its brand at 950 where pink anchored at 500, so the old step-named pairing list was already measuring the wrong pixels. The OKLCH override layer is emitted at a precision the generator RAISES until each triplet re-renders to its own hex fallback exactly, so the two representations are provably one pixel. `client/test/colorRamp.test.js` replaced its pinned-hex calibration test with one that PARSES themes.css and compares every ramp step, every OKLCH override and all four semantic blocks against the engine's output — verified to fail on both a one-channel hex edit and a single changed role reference — plus a second test pinning DEFAULT_MASTER to the default preset. Two silent inconsistencies surfaced and were fixed: `normaliseMasterConfig()` hardcoded `surfaceWash`/`borderTint` to true via `!== false` instead of falling back to DEFAULT_MASTER (harmless while the default WAS true), and `themeFromLegacyTokens()` now states both explicitly so migrating a pre-master palette keeps its tinted look rather than flattening because the shipped default changed. Also de-hardcoded: the PWA install banner carried a pasted copy of neutral-900/0/700 from the pink era, and the craft auditor's pure-grey message named a fixed neutral hue that is now seed-derived. `docs/design-system.md` §1.1–§2 rewritten from the generator's own `--markdown` output: the ramp tables are the shipped palette, the role columns are annotated as OUTPUT for this seed rather than as specification, and §2's tables are regenerated — all gated pairings now PASS, closing the two accepted 1.4.11 failures the pink palette shipped with (primary fill and focus ring at 1.86:1 on the canvas, now 9.74:1); only the switch-track miss remains, and it is a component decision carrying a `--border-interactive` outline. Verified: client 10/10, `node scripts/palette.mjs` reports all gated pairings PASS on both themes. **Still not done:** Prompt 3.5's own ACCEPTANCE lines remain unaudited, and `publishTheme()` re-runs `validatePalette()` on the stored blob but does not record the derived ramp steps in the audit diff — the before/after JSON still shows only the flattened sections plus the master config. |
| 19 | Catalog, variants, warehouses, batches | `DFD` 2.0, 19.0 | 4.1 | ⬜ |
| 20 | Media pipeline (R2 + local driver) | `technologyused.md` §7 | 4.2 | ⬜ |
| 21 | Pricing engine + dynamic splits | `PRD` §3.2 | 4.3, 4.7 | ⬜ |
| 22 | Search (Bengali + transliteration) | *(gap in v1.0)* | 4.4 | 🟡 Partial — **client-side "plan A" shipped 2026-08-29 at the owner's request; Prompt 4.4's server engine (REQUIREMENTS 1–5) is still entirely unbuilt and this row does not claim it.** The reported defect was that the TopBar search box did nothing: no typeahead as you type, and on submit it only mutated `/?q=` on the home page (dispatching an `explooro:search` event whose only effect — a grid re-filter far below the fold — was invisible from where the user was looking). What landed: (a) `client/src/components/search/SearchSuggest.js` — a self-contained typeahead dropdown attached to the existing TopBar `<form>`/`<input>` (debounced 200ms, min 2 chars, ↑/↓/Enter/Esc keyboard nav, thumb + title + category + ৳ price rows, "see all results" footer, no document-level listeners so it GCs with the TopBar's `replaceChildren` rebuild); (b) a real `/search?q=` route + `client/src/pages/SearchResultsPage.js` (thin catalog view reusing `ProductGrid` fed by `listProducts({ q })`, `noIndex` head, search-specific empty state); (c) TopBar submit now `navigate('/search?q=…')` from every page instead of the home-only `?q=` path; (d) `catalog.api.js` `searchSuggest()` wrapper on `GET /search/suggest`; (e) the mock handlers (`client/src/mocks/handlers/products.js`) for `/search`, `/search/suggest` and the `/products` `q` filter now share one `productMatchesQuery()` with a small seed Banglish→Bengali map (`shari`/`saree`→`শাড়ি`, etc.) so mock-mode search feels like the finished feature — the real `server/src/utils/transliterate.js` + `i18n_strings` table still owns this for live mode; (f) missing i18n keys `marketplace.search_results_for` / `marketplace.clear_search` (already referenced by HomePage's `updateSearchPill`, previously rendering a humanized slug) added to both locales, plus `marketplace.search_suggest.*` / `marketplace.search_page.*`; (g) `ProductGrid` gained optional `emptyTitle`/`emptyDescription` params (backward compatible); (h) the customer mobile-nav "Search" tab now points at `/search`; (i) `SearchSuggest` registered at `/dev/gallery` under "Product Discovery". Browser-verified on the live dev server: typeahead renders and matches Banglish ("shari" → শাড়ি products) and Bengali input, keyboard ↓↓+Enter opens the highlighted product, "see all" and the magnifier submit both land on `/search?q=` with correctly-pluralized result counts in en/bn, and the legacy `/?q=` deep link still works. **Not done:** everything server-side in Prompt 4.4 — `search.service.js`, the `postgres` tsvector driver with trigram typo tolerance and weighted ranking, the `meilisearch` stub, real `transliterate.js`, popular-query caching, zero-result logging, and grouped store/category results. The ACCEPTANCE lines ("shari" returns "শাড়ি" **via the engine**, single-char typo tolerance, <50ms typeahead, `SEARCH_DRIVER` swap) remain unmet. |
...
| 25 | Sourcing + profit calculator | `PRD` §3.2, §3.3 | 4.7 | ⬜ |
| 26 | Virtual storefront + OG images | `idea` §A, `PRD` §3.3 | 4.8 | ⬜ |
...
| 29 | Payments + idempotency + webhooks | `PRD` §3.5 | 5.3 | ⬜ |
