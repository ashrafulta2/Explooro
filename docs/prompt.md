...
2. Self-host Inter (latin subset) and Hind Siliguri (bengali subset) as woff2 with
...
| 18 | Theme & Color Studio + 5 presets | `technologyused.md` §L1 | 3.5 | ⬜ |
...
| 31 | Double-entry ledger + escrow | `DFD` 5.0 | 6.1 | ⬜ |
| 32 | Escrow release + clawback (incl. post-release) | `PRD` §3.2 | 6.2 | ⬜ |
| 33 | Payout engine with maker-checker | `idea` §AL.4 | 6.3 | ⬜ |
| 34 | **COD reconciliation** | `PRD` §5 gap 6 | 6.4 | ⬜ |
| 35 | Vault UI + finance dashboard | `PRD` §3.2 | 6.5 | ⬜ |
| 36 | Multi-carrier courier + tracking | `DFD` 6.0, `idea` §N | 7.1 | ⬜ |
| 37 | **Return & refund engine** | `DFD` 9.0, `idea` §F | 7.2 | ⬜ |
| 38 | **Dispute arbitration (3-way)** | `DFD` 9.0 | 7.3 | ⬜ |
| 39 | **Product approval & moderation** | `DFD` 10.0, `idea` §G | 7.4 | ⬜ |
| 40 | **KYC + blue tick + trust tiers** | `DFD` 11.0, `idea` §C | 7.5 | ⬜ |
| 41 | **Moderator dashboard** | `idea` §AL.5 | 7.6 | ⬜ |
| 42 | WebSocket chat + presence + offline | `DFD` 7.0 | 8.1, 8.4 | ⬜ |
| 43 | **Unified notification service** | `idea` §H | 8.2 | ⬜ |
| 44 | WhatsApp / Messenger bridge | `DFD` 20.0, `idea` §AD | 8.3 | ⬜ |
| 45 | **Sponsored ads engine** | `idea` §A | 9.1 | ⬜ |
| 46 | Coupons, vouchers, flash sales | `DFD` 17.0, `idea` §S | 9.2 | ⬜ |
| 47 | **Referral & network growth** | `DFD` 14.0, `idea` §Y | 9.3 | ⬜ |
| 48 | Coins, quests, leaderboard | `DFD` 13.0, `idea` §R, §X, §AE | 9.4 | ⬜ |
| 49 | Group buying / team purchase | `DFD` 16.0, `idea` §V | 9.5 | ⬜ |
| 50 | **Abandoned cart recovery** | `DFD` 12.0, `idea` §Q | 9.6 | ⬜ |
| 51 | Social seller kit, flyers, QR | `idea` §P | 9.7 | ⬜ |
| 52 | **Live stream commerce (+ tech decision)** | `DFD` 15.0, `idea` §U | 10.1 | ⬜ |
| 53 | **AI layer + concierge + sourcing chat** | `idea` §D, §W, §Z, §AB | 10.2, 10.3 | ⬜ |
| 54 | **Digital warranty & claims** | `idea` §AA | 10.4 | ⬜ |
| 55 | **Cross-seller bundling + surge** | `idea` §AC, §AF | 10.5 | ⬜ |
| 56 | **B2B escrow milestones** | `idea` §AG | 10.6 | ⬜ |
| 57 | **Open API + webhooks + SDK** | `idea` §AI | 10.7 | ⬜ |
| 58 | Content commerce, reels, academy, editor | `idea` §A, §T, §AL.6 | 10.8 | ⬜ |
| 59 | **Supplier dashboard + FEFO + routing** | `idea` §AL.1, `DFD` 19.0 | 11.1 | ⬜ |
| 60 | **Saler dashboard** | `idea` §AL.2 | 11.2 | ⬜ |
| 61 | **Customer portal** | `idea` §AL.3 | 11.3 | ⬜ |
| 62 | **Super Admin dashboard + analytics** | `idea` §AL.4 | 11.4 | ⬜ |
| 63 | **SEO: prerender + JSON-LD + sitemap** | `idea` §K | 11.5 | ⬜ |
| 64 | **PWA + offline queue** | `technologyused.md` §L1 | 11.6 | ⬜ |
