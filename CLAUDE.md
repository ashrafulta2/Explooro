...
the Status table in `README.md` — was, as of 2026-08-26: Phases 0, 1, 6, 7, 8, 9, 10, and 11 fully
done; Phases 2–5 partially done (see README's Status table for which individual prompts within
each); Phase 12 not started. As of 2026-08-28 that "fully done" label no longer holds cleanly for
9 and 10: active out-of-order polish is touching already-shipped customer surfaces — Coupons
...
| Change the platform's **default language** | Configuration, not code and not `.env`. Set it at **/admin/platform/language** (`platform.localization.view` / `.update`) — it writes the `localization` group in `platform_settings`. `VITE_DEFAULT_LOCALE` is only the pre-policy fallback. Delegate it by granting `platform.localization.update` (MEDIUM, delegable) from Access Grants |
...
| Turn the **popup genie animation** on/off, change how long it takes or how smooth it is | Configuration, not code. Set it at **/admin/platform/genie** (`platform.genie.view` / `.update`) — it writes the `genie` group in `platform_settings` (on/off, `duration_ms` 250–1500, `quality` light/balanced/smooth). The engine is `client/src/lib/genie.js`; `services/genieSettings.js` applies the policy at boot. Off = every Modal uses its plain CSS fade; reduced-motion users never see it regardless. Delegate it by granting `platform.genie.update` (MEDIUM, delegable). The bounds live in **two** places (`genie.js` `GENIE_LIMITS` and `genie.service.js` `DURATION_LIMITS`); `server/test/genieEffect.test.js` fails if they drift |
...
| Add or change an **admin page** | Read [`docs/super-admin-audit.md`](docs/super-admin-audit.md) §5 first — ten invariants, each written after the defect that broke it. In particular: the nav guard must equal the route guard (both permission AND module), module keys come from `server/src/config/modules.seed.json`, `--brand` is a fill token and `--text-brand` is the text one, and a missing i18n key renders a humanized slug instead of failing |
| Stop a page scrolling sideways / clip overflowing content | `overflow-x: clip`, **never** `overflow-x: hidden` (or `auto`/`scroll`) on `html`, `body`, `.app-shell`, `.app-shell__content` or a page root. `hidden` makes them scroll containers, so `position: sticky` stops working and the topbar scrolls away with the page. Fix the overflowing child first; clip is the backstop. Guarded by `client/test/homeOverflow.test.js` — see `docs/super-admin-audit.md` §5 invariant 10 |
