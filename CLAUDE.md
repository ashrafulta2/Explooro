...
| Change the colour the product BOOTS with | `DEFAULT_MASTER_PRESET` in `client/src/config/master-themes.js` — one line. `initTheme()` mounts it on every boot with no published palette, so it is the real default. Currently `midnight_slate`; `explooro_pink` is the alternate |
| Change the CSS baseline that paints before JS boots | `client/src/styles/themes.css` only, then `node scripts/palette.mjs` to re-verify contrast. This is a different thing from the default above: themes.css is still the engine's pink calibration ladder (`DEFAULT_MASTER` in `colorRamp.js` reproduces it, asserted by test), so a cold load shows it for the frames before `initTheme()` mounts the real default |
