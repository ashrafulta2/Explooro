...
| 3 — Admin Shell, Module Control & Theme Studio | ✅ **Complete** — 3.1–3.5 all done; +**Language & Default Locale** governance (`/admin/platform/language`, `platform.localization.view`/`.update`, matrix row 78 — the system default language is now a `platform_settings` row a Super Admin *or a user they grant* can change, not a build-time env var) (Module control backend & dynamic targeting hierarchy, 68-module grouped control panel with optimistic toggles, dependency cascades & DOM gate scanner, users & access governance with JIT/Maker-Checker approval inbox, audit trail explorer & diff viewer, and OKLCH master color engine with WCAG AA validation) |
...
| 11 — Role Dashboards, Analytics, SEO & PWA | ✅ **Complete** — 11.1–11.6 all done; +saved delivery address book (`/account/addresses`); +complete Supplier Account overhaul (Simple Mode 6 core pages + Advanced Mode 15-page suite with `supplier.css`); +complete **Saler Account Overhaul across all 19 pages** (Dashboard, Analytics, Cart Insights, Storefront Builder, My Curated Products `/saler/products`, Physical Shop Status `/saler/store-status`, Sourcing Catalog `/saler/sourcing`, Bundles `/saler/bundles`, Creative Studio `/saler/creative-studio`, Social Kit Flyer Studio `/saler/social-kit`, Sponsored Ad Campaigns `/saler/ads`, Live Stream Host Panel `/saler/live-studio`, Reseller Orders & Breakdown `/saler/orders` & `/saler/orders/:id`, Vault Balance `/saler/vault`, Withdrawals & Instant Payouts `/saler/vault/payouts`, 2-Tier Referrals `/saler/referrals`, Daily Quests & Monthly National Podium `/saler/quests` & `/saler/leaderboard`, Reseller Academy `/saler/academy`, Unified Multi-Channel Inbox `/saler/inbox` with `saler.css` design tokens, 100% EN/BN bilingual parity, and unit test coverage) — traceability matrix rows 59, 61 |
