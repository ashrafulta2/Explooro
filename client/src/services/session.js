...
import { api, setAccessToken, clearAccessToken } from '../core/api.js';
...
/**
 * Logs in with mobile number and password.
 */
...
export async function verifyOtp({ phone, otp, purpose = 'LOGIN' }) {
  try {
    const res = await api.post('/auth/verify-otp', { phone, code: otp, purpose });
    ...
    // REGISTER-purpose verification only marks the phone verified — it never issues a session
 ...
 */
export async function register({ phone, role, password, name }) {
