...
export function openClaimModal({ card, onSuccess = null } = {}) {
  ...
  formEl.id = 'claim-form';
  formEl.style.cssText = 'padding: 0; display: flex; flex-direction: column; gap: var(--space-4);';

  formEl.innerHTML = `
    <!-- Product Summary Banner -->
    <div style="background: var(--surface-2); border: 1px solid var(--border-subtle); border-radius: var(--radius-lg); padding: 10px 12px; display: flex; align-items: center; gap: 12px;">
      <img
    ...
    <!-- SLA Commitment Box -->
    <div style="background: var(--surface-2); border: 1px solid var(--border-subtle); border-radius: var(--radius-lg); padding: 10px 12px; font-size: 11px; color: var(--text-brand); line-height: 1.4;">
  ...
  const modal = Modal({
    title: `🛡️ ${t('warranty.file_claim_title')}`,
    description: `${t('warranty.claim_ref')}: #${certRef} • ${t('warranty.serial_number')}: ${card.serial_number || 'N/A'}`,
    content: formEl,
    size: 'md',
  });
